'''
   ...
   This is a modified version of Alastair Houghton's work found here:
   https://github.com/al45tair/mac_alias/blob/master/mac_alias/bookmark.py
...
# Bookmark keys
class BookmarkKey(IntEnum):
    #                  = 0x1003
    Path               = 0x1004   # Array of path components
    CNIDPath           = 0x1005   # Array of CNIDs
    FileProperties     = 0x1010   # (CFURL rp flags,
                                  #  CFURL rp flags asked for,
                                  #  8 bytes NULL)
    FileName           = 0x1020
    FileID             = 0x1030
    FileCreationDate   = 0x1040
    #                  = 0x1054   # ?
    #                  = 0x1055   # ?
    #                  = 0x1056   # ?
    #                  = 0x1101   # ?
    #                  = 0x1102   # ?
    TOCPath            = 0x2000   # A list of (TOC id, ?) pairs
    VolumePath         = 0x2002
    VolumeURL          = 0x2005
    VolumeName         = 0x2010
    VolumeUUID         = 0x2011   # Stored (perversely) as a string
    VolumeSize         = 0x2012
    VolumeCreationDate = 0x2013
    VolumeProperties   = 0x2020   # (CFURL vp flags,
                                  #  CFURL vp flags asked for,
                                  #  8 bytes NULL)
    VolumeIsRoot       = 0x2030   # True if volume is FS root
    VolumeBookmark     = 0x2040   # Embedded bookmark for disk image (TOC id)
    VolumeMountPoint   = 0x2050   # A URL
    #                  = 0x2070
    ContainingFolder   = 0xc001   # Index of containing folder in path
    UserName           = 0xc011   # User that created bookmark
    UID                = 0xc012   # UID that created bookmark
    WasFileReference   = 0xd001   # True if the URL was a file reference
    CreationOptions    = 0xd010
    URLLengths         = 0xe003   # See below
    #                  = 0xf017   # Localized name?
    #                  = 0xf022
    SecurityExtension  = 0xf080
    #                  = 0xf081
    ...
    @classmethod
    def get_key_by_code(cls, code):
        '''If code is present, return name, else code is returned'''
        for key in BookmarkKey:
            if code == key:
                return key
        return code


# URLLengths is an array that is set if the URL encoded by the
...
class Bookmark (object):
    ...
    def _get_item(cls, data, hdrsize, offset):
        ...
        if dtype == BMK_STRING:
            ...
            return datetime.datetime(2001, 1, 1) + secs
    ...
    def from_bytes(cls, data):
        ...
        while tocoffset != 0:
            ...
            for n in xrange(0,toccount):
                ...
                key = BookmarkKey.get_key_by_code(eid)
                toc[key] = cls._get_item(data, hdrsize, eoffset)
    ...
    def for_file(cls, path):
        ...
        toc = {
            BookmarkKey.Path: name_path,
            BookmarkKey.CNIDPath: cnid_path,
            BookmarkKey.FileCreationDate: crtime,
            BookmarkKey.FileProperties: fileprops,
            BookmarkKey.ContainingFolder: len(name_path) - 2,
            BookmarkKey.VolumePath: vol_path,
            BookmarkKey.VolumeIsRoot: vol_path == '/',
            BookmarkKey.VolumeURL: URL('file://' + vol_path),
            BookmarkKey.VolumeName: vol_name,
            BookmarkKey.VolumeSize: vol_size,
            BookmarkKey.VolumeCreationDate: vol_crtime,
            BookmarkKey.VolumeUUID: str(vol_uuid).upper(),
            BookmarkKey.VolumeProperties: volprops,
            BookmarkKey.CreationOptions: 512,
            BookmarkKey.WasFileReference: True,
            BookmarkKey.UserName: 'unknown',
            BookmarkKey.UID: 99,
        }

        if relcount:
            toc[BookmarkKey.URLLengths] = url_lengths
...
# Sample usage, reading bookmark data
# if __name__ == '__main__':
#     with open ("c:\\tests\\some_bookmark", 'rb') as bm_file:
#         data = bm_file.read() 
